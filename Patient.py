class Patient:
    ...
    def categorize(self):
        """
        Categorize patient based on treatment cost.
        Rule: Treatment cost > 10000 -> Special, else General
        """
...
class Hospital:
    """Manages a collection of patients."""
    ...
    def add_patient(self, patient_id, name, treatment_cost):
        """Add a new patient to the hospital records."""
        # Prevent duplicate patient IDs
        if any(p.patient_id == patient_id for p in self.patients):
            print(f"⚠️  Patient ID {patient_id} already exists. Use a unique ID.\n")
            return

        if treatment_cost < 0:
            print("⚠️  Treatment cost cannot be negative.\n")
            return
